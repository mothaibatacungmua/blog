...
totalTicks:{[z;b;e] count select from (z) where DateTime>=b, DateTime<(e+1)} / get all tick in a day
...
dpt:{[d;tbn;t]
    ...
    (.cm.stb[d;tbn]')p,'tbyd;}
genBars:{[d;tbn;sqtb;sts] / symbolPath, tableName, quoteTable, stepSize
    0N!d;
    qtb: `.[sqtb];
    fdate: .cm.fid[qtb];
    ldate: .cm.lad[qtb];
    ...
    (dpt[d;"/",tbn,"/";]')((sfw')ws);}
getTicks:{[ticktb;bd;ed]
    tot:count select from (ticktb) where DateTime>=bd, DateTime<ed; / Get total ticks from bd to ed
    ost:offsetTicks[ticktb;bd]; / Get total ticks to bd, make offset
    bfn:{[z;x] $[not .Q.qp[z];select[(x[0];(x[1] - x[0]))] from z;.Q.ind[z;x[0]+til (x[1]-x[0])]]};
    bfn[ticktb;(ost;tot)]}
\d .